...
- Run `pnpm run format` before every commit that changes TypeScript, and stage the result. For Markdown-only commits, inspect the diff directly and run `git diff --check`.
