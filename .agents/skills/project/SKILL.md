...
The harness axis lives in `packages/agent` and the device axis in the device adapters. Neither may import the other. `packages/projection` is the projection boundary and imports only `packages/interface`.
